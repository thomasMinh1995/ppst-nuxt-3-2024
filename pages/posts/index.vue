<template>
  ...
  <div class="grid grid-cols-4 gap-4">
    ...
    <NuxtLink v-else :to="`/posts/${p.id}`" v-for="p in posts" :key="p.id" class="post-item">
      {{ p.title }}
    </NuxtLink>
  </div>
...
const fetchPosts = async () => {
  ...
  const url = `https://dummyjson.com/posts?limit=${limit}&skip=${(currentPage.value - 1) * limit}&sortBy=${field}&order=${order}`;
